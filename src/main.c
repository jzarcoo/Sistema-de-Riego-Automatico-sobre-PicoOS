...
//     // Habilitar GPIO26 como ADC
//     adc_gpio_init(26);

//     // Seleccionar ADC0 (GPIO26)
//     adc_select_input(0);
...
/* Wrappers de ensamblador para excepciones (definidos en src/arch) */
extern void wrapper_svc(void);
extern void isr_pendsv(void);
...
int main() {
    ...
    // Espera a que la terminal USB se conecte
    while (!stdio_usb_connected()) {
        sleep_ms(100);
    }
    ...
    /* Registrar manejadores exclusivos de excepciones del Kernel */
    exception_set_exclusive_handler((enum exception_number)11, wrapper_svc); // SVC
    exception_set_exclusive_handler((enum exception_number)14, isr_pendsv);   // PendSV

    /* Inicializar el Process Stack Pointer (PSP) a 0 para el scheduler */
    __asm volatile ("msr psp, %0" : : "r" (0));
    ...
    while (1) {
        ...
        int valor = sys_read_soil_sensor();
